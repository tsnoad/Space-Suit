    ...
    difference() {
        union() {
            ...
            hull() {
                ...
                trans_chin_bar_from_locking_bar() translate([120+5+lck_arm_z0,55,5]) {
...
module chin_bar_side(right_hand_side=true) {
    ...
    intersection() {
        difference() {
            ...
            hull() {
                for(iy=[0,40]) translate([0,iy,0]) {
                    cylinder(r=bar_rad_outer-bev_s,h=10);
                    translate([0,0,bev_s]) cylinder(r=bar_rad_outer,h=10-2*bev_s);
                }
                
                for(ix=[0,1]) mirror([ix,0,0]) translate([-50+bar_rad_outer,55+30,0]) {
                    cylinder(r=50-bev_s,h=10);
                    translate([0,0,bev_s]) cylinder(r=50,h=10-2*bev_s);
                }
            }
            translate([0,0,-0.01]) {
                hull() {
                    ...
                    for(ix=[0,1]) mirror([ix,0,0]) translate([-50+bar_rad_outer,55+30,0]) cylinder(r=50-(bar_rad_outer-bar_rad_inner),h=50);
                }
                hull() {
                    for(iy=[0,40]) translate([0,iy,0]) {
                        cylinder(r1=bar_rad_inner+bev_s,r2=bar_rad_inner,h=bev_s);
                    }
                    for(ix=[0,1]) mirror([ix,0,0]) translate([-50+bar_rad_outer,55+30,0]) cylinder(r1=50-(bar_rad_outer-bar_rad_inner)+bev_s,r2=50-(bar_rad_outer-bar_rad_inner),h=bev_s);
                }
                translate([0,0,10-bev_xl+2*0.01]) hull() {
                    for(iy=[0,40]) translate([0,iy,0]) {
                        cylinder(r1=bar_rad_inner,r2=bar_rad_inner+bev_xl,h=bev_xl);
                    }
                    for(ix=[0,1]) mirror([ix,0,0]) translate([-50+bar_rad_outer,55+30,0]) cylinder(r1=50-(bar_rad_outer-bar_rad_inner),r2=50-(bar_rad_outer-bar_rad_inner)+bev_xl,h=bev_xl);
                }
            }
            
            for(ix=[0,1]) mirror([ix,0,0]) translate([0,55,5]) {
    ...
    translate([0,0,0]) difference() {
        ...
        union() {
            translate([0,0,vis_arm_z0]) hull() {
                ...
                translate([10,-30+5,0]) cube([-10+50+1,50,vis_arm_h]);
            }
            ...
            translate([45,-20-5,vis_arm_z0]) linear_extrude(height=vis_arm_h) {
            ...
            translate([0,0,vis_arm_z0]) hull() {
                translate([40+5,-40+5,0]) cube([5,2*30,5]);
                translate([55,-40+5,5-0.01]) cube([-55+40+l1,2*30,0.01]);
                translate([40+l2-4,-40+5,-10]) cube([4,2*30,15]);
            }
            
            //#translate([40+l2,-10+5-5/2,5-10+5]) cube([l1-l2,5,10]);
            for(iz2=[-22.5,0,22.5]) translate([40+l1,-10+5+iz2,5+5]) mirror([0,0,1]) {
        ...
        for(i=[-15,15]) {
            translate([40+l1-(l1-l2)/2,-10+5+i,0]) {
        ...
        union() {
            ...
            rotate([0,0,-(viewport_w_a/2-viewport_crn_w_a)]) translate([0,rad_vis_outer-thk,face_top-40]) rotate([-90,0,0]) difference() {
                linear_extrude(height=thk) polygon([
                    [0,0],
                    [sqrt(pow((viewport_crn_r+outset_bez_outer)+5,2)-pow(30-10+5,2)),0],
                    [sqrt(pow((viewport_crn_r+outset_bez_outer)+5,2)-pow(30-10+5,2)),-(30-10+5)],
                ]);
                translate([sqrt(pow((viewport_crn_r+outset_bez_outer)+5,2)-pow(30-10+5,2)),-(30-10+5),0]) {
    ...
    union() {
        difference() {
            ...
            translate([20,5,mnt_out_z0]) hull() {
                cylinder(r=40-2,h=mnt_out_h);
                cylinder(r=40,h=mnt_out_h-2);
            }
